...
- Latest FDs: **FD #46 (3 Aug 2026) — Real-Data Production Path OPENED (Option A)** — supersedes FD #44 for THIS SCOPE ONLY: AM/FO/II real-pipeline→API wiring (CS stays synthetic-labeled), SQLite persistence (run registry + snapshots + evidence lineage), single-user session auth (all /api/* except /api/health), provenance labels flip synthetic_demo → real_<source>_<timestamp> where real. NOT authorized: broker/execution, new pipeline stages/rules, CS real data, CIW Phase 11, multi-user, external deployment. Final production audit (prelaunch-close-beta split-lane) + Final Council are the release gates. Prior: FD #45 (3 Aug 2026 evening) — AM Findings Resolution + GAP-006 Fix (FSLR yfinance artifact; AMD genuine unwind; V0_TICKERS 5→9 → real EOD coverage 9/9). Chain: #1–44 + FD-CIW-001..016 + FD #45 + FD #46 (62 total).
