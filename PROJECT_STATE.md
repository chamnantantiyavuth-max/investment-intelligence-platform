...
- Latest FDs: **FD #54 (5 Aug 2026) — IIP Hermes AI Workforce Integration (APPROVED + MERGED):** org-pack demoted to subordinate operating standard; canonical state contract (two-axis Theme governance); new org-workflow authority grants (DATA/VALIDATION/RISK/GOVERNANCE Holds + IC Secretary gate + kanban movement, Q2 org-workflow scope only); 10 thin Principal profiles installed (Q1 Option A topology; Assistants = bounded subagents); repo-based kanban; 13 mapped templates (deep-dive paper + theme card rejected); no new cron; Option C dry-run pilot PASS 8/8. Prior: **FD #53 (4 Aug 2026) — FO/II unapproved formulas removed** (moat_score/signal_score deleted, −2σ trigger, trap 4 = MIXED, ADAPTER_VERSION v4) (70 total).
...
| FDs approved | #1–44 + FD-CIW-001..016 + FD #45..#54 (70 total) | 2026-08-05 |
...
**Hermes AI Workforce (FD #54) — INTEGRATED + MERGED (5 Aug 2026).** Operating standard + 10 principal profiles live;
pilot PASS 8/8. **Remaining decisions (optional follow-ups):** R3 source-map licensing backfill, L1 kanban
single-writer lock field, L2 delegation-retry clause (each = small amendment via 14-CHANGE-REQUEST). Unchanged:
A-01 (AM override/history scope — deferred per FD #52), C-04 state reconciliation (README/AGENTS checkpoints/
ROADMAP stale mirrors), C-05 vault fd-register rebuild (~46/70 rows), M-02 FO spec metadata. CIW remains PAUSED
(monitoring only; next decision point Q1-FY27 ~Oct 2026).
...
| fd_count | #1–53 + FD-CIW-001..016 + FD #54 (70 total) |
